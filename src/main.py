...
def prime_mod(pos:int,force:int) -> str:
	if force == 0:
		if pos >1000000000:
			return "please, enter number smaller then 1 000 000 000 or use force-mode /P/n (if you really want to wait so long)"
	if pos > 0:
...
class MainHandler(RequestHandler):
	def get(self, data:str)-> int:
		data = str(data)
		mode = data[0]
		if mode == 'f':
			pos = data[2:]
			pos = int(pos) - 1
			result = fibonacchi_mod(pos)
			self.write({"num" : result})
		elif mode == 'p':
			pos = data[2:]
			pos = int(pos)
			result = prime_mod(pos,0)
			self.write({"num" : result})
		elif mode == 'P':
			pos = data[2:]
			pos = int(pos)
